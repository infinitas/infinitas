...
/* SVN FILE: $Id$ */
/**
 * Tree behavior - modified by Jamie Nay to add better scope support.
 *
 * Tree behavior class.
 *
 ...
 */
	function beforeSave(&$Model) {
		// Correct the scope of the behavior settings if required.
 ...
 */
	function getpath(&$Model, $id = null, $fields = null, $recursive = null) {
		...
		$result = $Model->find('first', array('conditions' => array($Model->escapeField() => $id), 'fields' => array($left, $right, $scopeForeignKey), 'recursive' => $recursive));
		if ($result) {
		...
		$results = $Model->find('all', array(
			'conditions' => array($scope, $scopeForeignKey => $item[$scopeForeignKey], $Model->escapeField($left) . ' <=' => $item[$left], $Model->escapeField($right) . ' >=' => $item[$right]),
 ...
 */
	function recover(&$Model, $mode = 'parent', $missingParentAction = null) {
		...
		if ($mode == 'parent') {
			...
			$count = 1;
			foreach ($Model->find('all', array('conditions' => $scope, 'fields' => array($Model->primaryKey), 'order' => $Model->alias . '.' . $left)) as $array) {
				$Model->id = $array[$Model->alias][$Model->primaryKey];
				$lft = $count++;
				$rght = $count++;
				$Model->save(array($left => $lft, $right => $rght), array('callbacks' => false));
			}
			foreach ($Model->find('all', array('conditions' => $scope, 'fields' => array($Model->primaryKey, $parent), 'order' => $Model->alias . '.' . $left)) as $array) {
				$Model->create();
				$Model->id = $array[$Model->alias][$Model->primaryKey];
