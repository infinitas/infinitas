 ...
 */
	public function testValidateEitherOr() {
		...
		$expected = array('field_1' => array('pick one, only one'));
		 ...
		 */
		$this->User->validate['field_1']['validateEitherOr']['allowEmpty'] = true;
		...
		/**
		 * cant use required for this rule
		 */
		unset($this->User->validate['field_1']['validateEitherOr']['allowEmpty']);
		...
		$data['User'] = array('field_1' => '', 'field_2' => 'bar');
		$this->User->set($data); $this->User->validates();
		$this->assertEqual($this->User->validationErrors, $expected);

		$data['User'] = array('field_1' => 'bar', 'field_2' => '');
		$this->User->set($data); $this->User->validates();
		$this->assertEqual($this->User->validationErrors, array());

		$data['User'] = array('field_1' => 'bar', 'field_2' => 'foo');
		$this->User->set($data); $this->User->validates();
		$this->assertEqual($this->User->validationErrors, $expected);
	}
