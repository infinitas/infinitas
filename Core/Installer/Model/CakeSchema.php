	...
	class CakeSchema extends Object {
		...
		function __construct($options = array()) {
			...
			if (empty($options['path'])) {
				if (is_dir(APP . 'Config' . DS . 'schema')) {
					$this->path = APP . 'Config' . DS . 'schema';
				}

				else {
					$this->path = APP . 'Config' . DS . 'sql';
				}
			}
				...
				foreach ($models as $model) {
					...
					if (is_object($Object) && isset($Object->useTable) && $Object->useTable !== false) {
						...
						if (in_array($table, $currentTables)) {
							...
							if (empty($tables[$useTable])) {
								$tables[$useTable] = $this->__columns($Object);
								$tables[$useTable]['indexes'] = $db->index($Object);
		...
		function compare($old, $new = null) {
			...
			if (is_array($new) && isset($new['tables'])) {
				$new = $new['tables'];
			}
			...
			if (is_array($old) && isset($old['tables'])) {
				$old = $old['tables'];
			}
			...
			foreach ($new as $table => $fields) {
				...
				foreach ($fields as $field => $value) {
					...
					if (isset($add[$table][$field])) {
						...
						$column = array_search($field, $wrapper);
						
						if ($column && isset($wrapper[$column - 1])) {
