<?php
	...
	class CakeRelease extends Object {
		...
		public function run($direction, $basePrefix = '', $sample = false) {
			if (!in_array($direction, array('up', 'down'))) {
				throw new MigrationException(
					$this,
					sprintf(__d('migrations', 'Migration direction (%s) is not one of valid directions.'), $direction),
					E_USER_NOTICE
				);
			}
			...
			try {
				...
				$this->_run();
				$this->_clearCache();
				$this->__installData($basePrefix, $sample);
		...
		protected function __rewriteMigrations($basePrefix = '') {
			$newMigrations = array();
			...
			$this->migration = $newMigrations;
		}

		private function __getTableName($className) {
			$fullClassName = (isset($this->plugin) ? $this->plugin . '.' : '' ) . $className;

			App::import('Model', $fullClassName);
			$classVars = get_class_vars($className);

			if (empty($classVars['useTable'])) {
				return Inflector::tableize($className);
			}

			return $classVars['useTable'];
		}

		private function __getTablePrefix($tableName, $basePrefix = '') {
			...
			return $basePrefix . $classVars['tablePrefix'];
		}
			...
			foreach ($this->migration[$this->direction] as $type => $info) {
				switch ($type) {
					...
					default:
						throw new MigrationException(
							$this,
							sprintf(__d('migrations', 'Migration action type (%s) is not one of valid actions type.'), $type),
							E_USER_NOTICE
						);
				}
		...
		protected function _renameTable($type, $tables) {
			foreach ($tables as $oldName => $newName) {
				...
				if (!in_array($oldName, $sources)) {
					throw new MigrationException($this, sprintf(__d('migrations', 'Table "%s" does not exists in database.'), $oldName));
				}

				else if (in_array($newName, $sources)) {
					throw new MigrationException($this, sprintf(__d('migrations', 'Table "%s" already exists in database.'), $newName));
				}
			...
			foreach ($tables as $table => $fields) {
				...
				foreach ($fields as $field => $col) {
					...
					if ($type !== 'add' && !isset($tableFields[$field])) {
						...
						throw new MigrationException($this, sprintf(__d('migrations', 'Field "%s" does not exists in "%s".'), $field, $table));
					}
