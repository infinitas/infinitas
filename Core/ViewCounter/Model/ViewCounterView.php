...
class ViewCounterView extends ViewCounterAppModel {
/**
	...
	public function reportYearOnYear($conditions) {
		...
		$viewCountsByYear = $this->find(
			...
			array(
				'fields' => array(
					$this->alias . '.id',
					isset($conditions[$this->alias . '.model']) ? $this->alias . '.model' : '',
					isset($conditions[$this->alias . '.foreign_key']) ? $this->alias . '.foreign_key' : '',
					$this->alias . '.year',
					'sub_total',
					$this->alias . '.created'
				),
	...
	public function reportPopularModels() {
		...
		$models = $this->find(
			...
			array(
				...
				'conditions' => array(
					$this->alias . '.year BETWEEN ? AND ?' => array(
						date('y', mktime(0, 0, 0, date('m') - 12)),
						date('y', time()),
					),
					$this->alias . '.month >= ' => date('m', mktime(0, 0, 0, date('m') - 12))
				),
				'group' => array(
					$this->alias . '.model'
				)
 ...
 */
	public function reportByRegion($conditions = array(), $limit = 24) {
		...
		$conditions[$this->alias . '.country_code != \'-\''] = '';
