<?php
	...
	if (empty($chartData)) {
		...
		$chartData = ClassRegistry::init('ViewCounter.ViewCounterView')->reportLastTwoWeeks(array(
			'model' => $model,
	...
	echo $this->Html->tag('p', __d('view_counter', '%s views per day', round($chartData['stats']['median'], 2)));