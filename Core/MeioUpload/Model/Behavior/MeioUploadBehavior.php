...
// @codingStandardsIgnoreStart
	...
	class MeioUploadBehavior extends ModelBehavior {
		 ...
		 * @var array
		 **/
		protected $_imageTypes = array('image/jpeg', 'image/pjpeg', 'image/png', 'image/gif', 'image/bmp', 'image/x-icon', 'image/vnd.microsoft.icon');
		...
		/**
		 * Setup the behavior. It stores a reference to the model, merges the default
		 * options with the options for each field, and setup the validation rules.
		 *
		 * @param $model Object
		...
		function setup($model, $settings = array()) {
			...
			foreach ($settings as $field => $options) {
				...
				if ($options['useTable'] && !$model->hasField($field)) {
					trigger_error(
						sprintf(
							__d('meio_upload', 'MeioUploadBehavior Error: The field "%s" doesn\'t exists in the model "%s".'),
							$field,
							$model->alias
						),
						E_USER_WARNING
		...
		function uploadCheckFieldName($model, $data) {
			foreach ($data as $fieldName => $field) {
				...
				$this->log(
					sprintf(
						__d('meio_upload', 'MeioUploadBehavior Error: The field "%s" wasn\'t declared as part of the MeioUploadBehavior in model "%s".'),
						$fieldName,
						$model->alias
					)
				);
			...
			foreach ($data as $fieldName => $field) {
				...
				if (empty($field['remove']) || empty($field['name'])) {
					...
					if (!is_dir($options['dir'])) {
						if ($options['createDirectory']) {
							...
							if (!$folder->create($options['dir'])) {
								trigger_error(
									sprintf(
										__d('meio_upload', 'MeioUploadBehavior Error: The directory %s does not exist and cannot be created.'),
										$options['dir']
									),
									E_USER_WARNING
								);
						...
						trigger_error(sprintf(__d('meio_upload', 'MeioUploadBehavior Error: The directory %s does not exist.'), $options['dir']), E_USER_WARNING);
						return false;
					}
					...
					if (!is_writable($options['dir'])) {
						trigger_error(
							sprintf(
								__d('meio_upload', 'MeioUploadBehavior Error: The directory %s isn\'t writable.'),
								$options['dir']
							),
							E_USER_WARNING
						);
		...
		function _uploadFile($model, $data = null) {
			...
			foreach ($this->__fields[$model->alias] as $fieldName => $options) {
				...
				if ($options['useTable'] == false) {
					...
					if ((count($options['thumbsizes']) > 0) && count($options['allowedExt']) > 0 &&
						in_array($data[$model->alias][$fieldName]['type'], $this->_imageTypes)) {
		...
		*/
		...
		function _unsetDataFields($modelName, $fieldName, $data, $options) {
			unset(
				...
				$data[$modelName][$options['fields']['dir']],
				$data[$modelName][$options['fields']['filesize']],
				$data[$modelName][$options['fields']['mimetype']]
			);
