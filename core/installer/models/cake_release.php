	...
	class CakeRelease extends Object {
		...
		public function run($direction, $basePrefix = '', $sample = false) {
			if (!in_array($direction, array('up', 'down'))) {
				throw new MigrationException(
					$this,
					sprintf(__d('migrations', 'Migration direction (%s) is not one of valid directions.', true), $direction),
					E_USER_NOTICE
				);
			}
			...
			foreach ($this->migration[$this->direction] as $type => $info) {
				switch ($type) {
					...
					default:
						throw new MigrationException(
							$this,
							sprintf(__d('migrations', 'Migration action type (%s) is not one of valid actions type.', true), $type),
							E_USER_NOTICE
						);
				}
		...
		protected function _dropTable($type, $tables) {
			foreach ($tables as $table) {
				if (!in_array($table, $this->db->listSources())) {
					throw new MigrationException($this, sprintf(__d('migrations', 'Table "%s" does not exists in database.', true), $table));
				}
