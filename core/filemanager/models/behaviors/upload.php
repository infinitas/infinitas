<?php
	...
	class UploadBehavior extends ModelBehavior {
		...
		protected $__filesToRemove = array();
		/**
		...
		function _resizePhp(&$model, $field, $path, $style, $geometry) {
			...
			$src = $createHandler = $outputHandler = null;
			...
			if ($src) {
				...
				$options = array('srcW' => $srcW, 'srcH' => $srcH, 'destW' => $destW, 'destH' => $destH);
				switch($resizeMode){
					case 'best':
						$ratio = $this->_ratioBest($resizeMode, $options);
						$resizeW = $srcW * $ratio;
						$resizeH = $srcH * $ratio;
						break;

					case 'band':
						$ratio = $this->_ratioBand($resizeMode, $options);
						$resizeW = $srcW * $ratio;
						$resizeH = $srcH * $ratio;
						break;
		...
		/**
		 * @breif calculate the 'best' ratio
		 * 
		 * @param <type> $type
		 * @param <type> $params
		 */
		function _ratioBest($type, $options = array()){
			// "best fit" mode
			if ($options['srcW'] > $options['srcH']) {
				if ($options['srcH'] / $options['destH'] > $options['srcW'] / $options['destW']){
					$ratio = $options['destW'] / $options['srcW'];
				}

				else{
					$ratio = $options['destH'] / $options['srcH'];
				}
			}
		...
		/**
		 * @breif calculate the 'band' ratio
		 *
		 * @param <type> $type
		 * @param <type> $options
		 * @return <type>
		 */
		function _ratioBand($type, $options = array()){
			if ($options['srcW'] > $options['srcH']){
				$ratio = $options['destW'] / $options['srcW'];
			}
		...
		function _prepareFilesForDeletion(&$model, $field, $data, $options) {
			...
			$this->__filesToRemove[$model->alias][] =
				ROOT . DS . APP_DIR . DS . $this->settings[$model->alias][$field]['path'] . 
				$data[$model->alias][$options['fields']['dir']] . DS . $data[$model->alias][$field];

			foreach ($options['thumbsizes'] as $style => $geometry) {
				$this->__filesToRemove[$model->alias][] = 
					ROOT . DS . APP_DIR . DS . $this->settings[$model->alias][$field]['path'] .
					$data[$model->alias][$options['fields']['dir']] . DS . $style . '_' . $data[$model->alias][$field];
			}
