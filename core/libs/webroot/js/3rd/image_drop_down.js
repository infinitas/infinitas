...
(function($){
	...
	$.fn.dd = function(options){
		...
		ddList = (($(this).attr("size") > 0) || ($(this).attr("multiple") == true)) ? true
				: false;
		if(ddList){
			...
			childnodes.each(function(current){
				...
				// OPTGROUP
					if(currentOption.nodeName == "OPTGROUP"){
						aTag += "<div class='opta'>";
						aTag += "<span style='font-weight:bold;font-style:italic; clear:both;'>"
								+ $(currentOption).attr("label") + "</span>";
						var optChild = $(currentOption).children();
						optChild
								.each(function(currentopt){
									var currentOptOption = optChild[currentopt];
									var aid = aidoptfix + "_" + (current) + "_"
											+ (currentopt);
									var arrow = $(currentOptOption).attr(
											"title");
									arrow = (arrow.length == 0) ? ""
											: '<img src="' + arrow + '" align="left" /> ';
									var sText = $(currentOptOption).text();
									var sValue = $(currentOptOption).val();
									var sEnabledClass = ($(currentOptOption)
											.attr("disabled") == true) ? "disabled"
											: "enabled";
									a_array[aid] = {
										html : arrow + sText,
										value : sValue,
						...
						var sEnabledClass = ($(currentOption).attr("disabled") == true) ? "disabled"
								: "enabled";
						a_array[aid] = {
							html : arrow + sText,
							value : sValue,
		...
		function createTitleDiv(){
			...
			var sDiv = '<div id="' + titleid + '" class="' + styles.ddTitle
					+ '"';
			sDiv += '>';
			sDiv += '<span id="' + arrowid + '" class="' + styles.arrow
					+ '"></span><span class="textTitle" id="' + titletextid
					+ '">' + arrow + sText + '</span></div>';
			return sDiv;
		};
		...
		function createDropDown(){
			...
			if(allOptions.length > options.visibleRows){
				var margin = parseInt($("#" + childid + " a:first").css(
						"padding-bottom"))
						+ parseInt($("#" + childid + " a:first").css(
								"padding-top"));
				var iHeight = ((options.rowHeight) * options.visibleRows)
						- margin;
			...
			if($("#" + elementid).attr("disabled") == true){
				...
				$("#" + childid + " a.enabled").bind(
						...
						function(event){
							...
							if(!ddList){
								...
								var sText = (options.showIcon == false) ?
									$(this).text() : $(this).html();
								setTitleText(sText);
								closeMe();
							};
				...
				$("#" + titleid).bind("mouseout", function(evt){setInsideWindow(false);});
			};
		};
		...
		function manageSelection(obj){
			...
			if(selectedA != undefined){
				var oldIndex = ((actionSettings.oldIndex == undefined) || (actionSettings.oldIndex == null)) ? a_array[selectedA].index
						: actionSettings.oldIndex;
			};
			...
			if(ddList){
				...
				if($("#" + elementid).attr("multiple") == true){
					if(keyCode == 17){
						...
						if(keyCode == 16){
							$("#" + childid + " a.selected").removeClass(
									"selected");
							$(obj).addClass("selected");
							// shift
							var currentSelected = $(obj).attr("id");
							var currentIndex = a_array[currentSelected].index;
							for( var i = Math.min(oldIndex, currentIndex); i <= Math
									.max(oldIndex, currentIndex); i++){
		...
		function setOutOfVision(){
			var sId = getPostID("postElementHolder");
			$("#" + elementid).after(
					"<div style='height:0px;overflow:hidden;position:absolute;' id='"
							+ sId + "'></div>");
		...
		function next(){
			...
			for( var current = 0; current < allAs.length; current++){
				...
				if($(currentA).hasClass("selected")
						&& (current < allAs.length - 1)){
					$("#" + childid + " a.selected").removeClass("selected");
					$(allAs[current + 1]).addClass("selected");
					// manageSelection(allAs[current+1]);
					var selectedA = $("#" + childid + " a.selected").attr("id");
					if(!ddList){
						var sText = (options.showIcon == false) ? a_array[selectedA].text
								: a_array[selectedA].html;
						setTitleText(sText);
					}
					if(parseInt(($("#" + selectedA).position().top + $(
							"#" + selectedA).height())) >= parseInt($(
							"#" + childid).height())){
						$("#" + childid).scrollTop(
								($("#" + childid).scrollTop())
										+ $("#" + selectedA).height()
		...
		function setValue(){
			...
			if(allSelected.length == 1){
				...
				if(allSelected.length > 1){
					var alls = $("#" + elementid + " > option:selected")
							.removeAttr("selected");
		...
		function openMe(){
			...
			if($("#" + childid).css("display") == "none"){
				selectedValue = a_array[$("#" + childid + " a.selected").attr(
						"id")].text;
				$(document).bind("keydown", function(event){
					var keyCode = event.keyCode;
					if((keyCode == 39) || (keyCode == 40)){
		...
		function checkMethodAndApply(){
			...
			if($("#" + elementid).attr("onchange") != undefined){
				var currentSelectedValue = a_array[$(
						"#" + childid + " a.selected").attr("id")].text;
