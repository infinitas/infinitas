<?php
	...
	class LogableBehavior extends ModelBehavior {
		...
		function findUserActions($Model, $userId, $params = array()) {
			...
			$ceheck = isset($this->user) && isset($this->user[$this->UserModel->alias][$this->UserModel->primaryKey]) &&
				$userId == $this->user[$this->UserModel->alias][$this->UserModel->primaryKey] &&
			...
			if ($check) {
				...
				$user = $this->UserModel->find(array($this->UserModel->primaryKey => $userId));
			...
			foreach ($data as $key => $row) {
				...
				$check = isset($one[$this->settings[$Model->alias]['classField']]) &&
					isset($one['action']) && isset($one['change']) &&
					isset($one[$this->settings[$Model->alias]['foreignKey']]);

				if ($check) {
					if ($one['action'] == 'edit') {
						$result[$key]['Log']['event'] .= ' edited ' . $one['change'] . ' of ' . low($one[$this->settings[$Model->alias]['classField']]) .
							'(id ' . $one[$this->settings[$Model->alias]['foreignKey']] . ')';
					} 
					
					else if ($one['action'] == 'add') {
						$result[$key]['Log']['event'] .= ' added a ' . low($one[$this->settings[$Model->alias]['classField']]) .
							'(id ' . $one[$this->settings[$Model->alias]['foreignKey']] . ')';
					} 
					
					else if ($one['action'] == 'delete') {
						$result[$key]['Log']['event'] .= ' deleted the ' . low($one[$this->settings[$Model->alias]['classField']]) .
							'(id ' . $one[$this->settings[$Model->alias]['foreignKey']] . ')';
					}
				}

				elseif (isset($one[$this->settings[$Model->alias]['classField']]) && isset($one['action']) &&
						isset($one[$this->settings[$Model->alias]['foreignKey']])) { // have model,model_id and action
					if ($one['action'] == 'edit') {
						$result[$key]['Log']['event'] .= ' edited ' . low($one[$this->settings[$Model->alias]['classField']]) .
							'(id ' . $one[$this->settings[$Model->alias]['foreignKey']] . ')';
					}
		...
		function afterSave($Model, $created) {
			...
			if (isset($this->Log->_schema['change'])) {
				...
				$logData['Log']['changes'] = sizeof($changedFields);
				if ($logData['Log']['changes'] == 0) {
					return true;
				}
		...
		function _saveLog($Model, $logData, $title = null) {
			...
			$check = isset($this->Log->_schema[$this->settings[$Model->alias]['foreignKey']]) &&
				!isset($logData['Log'][$this->settings[$Model->alias]['foreignKey']]);
			if ($check) {
				if ($Model->id) {
