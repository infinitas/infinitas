	...
	class MeioUploadBehavior extends ModelBehavior {
		...
		function setup($model, $settings = array()) {
			...
			foreach ($settings as $field => $options) {
				...
				if ($options['useTable'] && !$model->hasField($field)) {
					trigger_error(
						sprintf(
							__d('meio_upload', 'MeioUploadBehavior Error: The field "%s" doesn\'t exists in the model "%s".', true),
							$field,
							$model->alias
						),
						E_USER_WARNING
		...
		function uploadCheckFieldName($model, $data) {
			foreach ($data as $fieldName => $field) {
				...
				$this->log(
					sprintf(
						__d('meio_upload', 'MeioUploadBehavior Error: The field "%s" wasn\'t declared as part of the MeioUploadBehavior in model "%s".', true),
						$fieldName,
						$model->alias
					)
				);
			...
			foreach ($data as $fieldName => $field) {
				...
				if (empty($field['remove']) || empty($field['name'])) {
					...
					if (!is_dir($options['dir'])) {
						if ($options['createDirectory']) {
							...
							if (!$folder->create($options['dir'])) {
								trigger_error(
									sprintf(
										__d('meio_upload', 'MeioUploadBehavior Error: The directory %s does not exist and cannot be created.', true),
										$options['dir']
									),
									E_USER_WARNING
								);
					...
					if (!is_writable($options['dir'])) {
						trigger_error(
							sprintf(
								__d('meio_upload', 'MeioUploadBehavior Error: The directory %s isn\'t writable.', true),
								$options['dir']
							),
							E_USER_WARNING
						);
