<?php
	...
	class AppModel extends Model {
		 ...
		 */
		public function rawSave($data, $options = array()) {
			$options = array_merge(
				array(
					'validate' => true,
					'chunk' => 500,
			...
			$primaryKeySchema = $this->schema($this->primaryKey);

			if($primaryKeySchema['type'] == 'string' && !in_array($this->primaryKey, $fields)) {
				$fields[] = $pk = $this->primaryKey;
			...
			foreach(array_chunk($data, $options['chunk']) as $dataChunk) {
				$query = sprintf(
					$this->saveTemplate,
					$this->fullTableName(null, false),
					'`' . implode('`, `', $schema) . '`',
					implode(', ', $dataChunk)
				);
				$saved = $saved && $DB->rawQuery($query);
			}
